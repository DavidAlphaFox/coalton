...
either because FROM-TYPE is signed and TO-TYPE is unsigned, or because FROM-TYPE is wider than TO-TYPE."
    `(define-instance (TryInto ,from-type String ,to-type)
...
(coalton-toplevel
  (define-instance (TryInto Integer String Single-Float)
    (define (tryInto x)
  ...
  (define-instance (TryInto Integer String Double-Float)
    (define (tryInto x)
...
(cl:eval-when (:compile-toplevel :load-toplevel)
  (cl:defmacro integer-tryinto-float (integer lisp-float float pow)
    `(define-instance (TryInto ,integer String ,float)
       (define (tryInto x)
