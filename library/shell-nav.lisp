(coalton-library/utils:defstdlib-package #:coalton-library/shell-nav
  (:use
   #:coalton
...
;; reader or state monad, carries the current working directory, 

;; maybe to move to shell or nav
(coalton-toplevel

  (declare cwd (Unit -> Pathname))
  (define (cwd)
    "Returns the current working directory."
    (lisp Pathname ()
      (uiop:getcwd)))

  (declare pwd (Unit -> Pathname))
  (define (pwd)
    "Returns the current working directory."
    (lisp Pathname ()
      (uiop:getcwd)))
  ...
  (declare back (Unit -> Pathname))
  (define (back)
    "Returns the current working directory one level up."
    (cd "..")
    (pwd))

  (declare cwd-relative-pathname (String -> Pathname))
...
(coalton-toplevel
  ...
  (declare cwd-file? (Pathname -> Boolean))
  (define (cwd-file? name)
    "Checks whether a file exists."
    (let file = (cwd-relative-pathname name))
    (file-exists? file) #+igno(lisp Boolean (file)
         (cl:if (cl:probe-file file)
                cl:t
                cl:nil)))

  ;(declare concat-files ((List String) -> String -> Unit))
  #+ignore(define (concat-files inputs output)
...
#+sb-package-locks
(sb-ext:lock-package "COALTON-LIBRARY/SHELL-NAV")
